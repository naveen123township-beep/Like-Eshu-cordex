...
def handle_requests():
    ...
    try:
        def process_request():
            ...
            # FIXED: Ensure before_like is captured correctly
            ...
            # FIXED: Logic to fill NA values from data_after if external API failed
            after_like = int(data_after.get('AccountInfo', {}).get('Likes', 0))
            player_uid = int(data_after.get('AccountInfo', {}).get('UID', uid))
            player_name = str(data_after.get('AccountInfo', {}).get('PlayerNickname', ''))
            
            # If the external API failed (NA), use the data from the game response
            final_level = level if level != "NA" else data_after.get('AccountInfo', {}).get('Level', "NA")
            final_region = region if region != "NA" else server_name_used
